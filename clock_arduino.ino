...
int full_data_set [5] = {10, 22, 3, 1, 5}; //초,분,시,요일,일,월
...
const byte clockStyle = CLOCK_STYLE_SIMPLE_MIX;
...
void loop() {
  /*
    read - time and date and cpu temp
    send - temp and humd
  */
  ...
  if (!(isnan(t) || isnan(h))) {
    ...
    Serial.println(data_total); //온도 습도 데이터 합산
  }
  //if (Serial.available()) {
  int full_date_data = Serial.parseInt();
  //Serial.println(full_date_data);

  if (full_date_data %10 == 2) {//2-2
    full_date_data /= 10;
    full_data_set[0] = full_date_data % 100;//min
    full_date_data =  full_date_data / 100;

    full_data_set[1] = full_date_data % 100;//hour
    // full_date_data =  full_date_data / 100;
  }
  else if (full_date_data%10 == 1) {//1-1
    full_date_data /= 10;
    full_data_set[2] = full_date_data % 100; //day
    full_date_data =  full_date_data / 100;

    full_data_set[3] = full_date_data % 100; //month
    //  full_date_data =  full_date_data / 100;

  }
  else if (full_date_data %10== 4) {//3-4
    full_date_data /= 10;
    full_data_set[4] = full_date_data % 10; //day
    // full_date_data =  full_date_data / 100;


  }
  /*
    for (int cnnt = 0; cnnt < 6; cnnt++)  {
    full_data_set[cnnt] = full_date_data % 100;
    full_date_data /= 100;
    }
  ...
  */
  ...
  //new_cpu_temp = cpu_temp / 10;
  //new_cpu_temp = new_cpu_temp * 0.1;
...
void drawClock() {
  ...
  do {
    ...
    if (clockStyle == CLOCK_STYLE_SIMPLE_MIX) {
      ...
      showTimePin(centerY, centerY, 0.1, 0.4, full_data_set[1] * 5 + (int)(full_data_set[0] * 5 / 60));
      showTimePin(centerY, centerY, 0.1, 0.70, full_data_set[0]);
      ...
      u8g.setDefaultForegroundColor();
      u8g.setFontPosTop();
      int week_data = full_data_set[4];
      u8g.drawStr(centerX + 3, 23, (const char*)pgm_read_word(&(weekString[week_data])) );
      // int add_date = full_data_set[5] * 100 + full_data_set[4];
      //  u8g.drawStr(centerX + 28, 23, (const char*)pgm_read_word(&(full_data_set[3]))add_date);

      String strDate = String("");
      char buff[6];
      if (full_data_set[1] < 10) strDate += "0";
      strDate += full_data_set[1];
      ...
      if(colon)
	    ...
	    strDate += " ";
      if (full_data_set[0] < 10) strDate += "0";
      strDate += full_data_set[0];
      strDate.toCharArray(buff, 6);
      buff[5] = 0x00;
